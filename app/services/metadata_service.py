...
class MetadataExtractor:
    ...
    def compute_speaker_stats(speaker_segments: list[dict]) -> dict:
        ...
        for seg in speaker_segments:
            ...
            stats[spk]["longest_monologue"] = max(stats[spk]["longest_monologue"], duration)

            # first spoken
            if stats[spk]["first_spoke_at"] is None:
                stats[spk]["first_spoke_at"] = seg["start"]
            else:
                stats[spk]["first_spoke_at"] = min(stats[spk]["first_spoke_at"], seg["start"])

            # last spoken
            if stats[spk]["last_spoke_at"] is None:
                stats[spk]["last_spoke_at"] = seg["end"]
            else:
                stats[spk]["last_spoke_at"] = max(stats[spk]["last_spoke_at"], seg["end"])
        ...
        for _spk, s in stats.items():
            s["avg_segment_length"] = s["total_speaking_time"] / max(s["segment_count"], 1)
            s["wpm"] = (s["total_words"] / s["total_speaking_time"] * 60) if s["total_speaking_time"] > 0 else 0
            s["speaking_ratio"] = s["total_speaking_time"] / max(total_audio_talk_time, 1.0)
            s["word_ratio"] = s["total_words"] / max(total_audio_words, 1.0)

        return dict(stats)

    @staticmethod
    def compute_conversation_stats(speaker_segments: list[dict], diarization_segments: list[dict]) -> dict:
        """
        ...
        total_words = sum(len((seg.get("text") or "").split()) for seg in speaker_segments)
