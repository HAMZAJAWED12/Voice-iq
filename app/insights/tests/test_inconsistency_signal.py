...
def _utt(
    ...
    sentiment = SentimentInput(label=sentiment_label, score=sentiment_score) if sentiment_label is not None else None
...
def test_neutral_session_produces_no_inconsistency():
    ...
    result = InsightInconsistencyEngine.assess(session, _build_analytics(), aggregated)
...
def test_sentiment_text_mismatch_fires_when_label_disagrees_with_text():
    ...
    result = InsightInconsistencyEngine.assess(session, _build_analytics(), AggregatedSignals())
...
def test_abrupt_emotional_reversal_fires_within_window():
    ...
    reversal_windows = [w for w in result.windows if "from_emotion" in w.evidence]
